...
def signal(f,n):
    return np.cos(2*np.pi*f*n*1/fs)+0.2j*np.cos(4*np.pi*f*n*1/fs)
...
signalRLn,  = plt.plot([],[],'b-',linewidth=3,alpha=0.6)
signalILn,  = plt.plot([],[],'r-',linewidth=3,alpha=0.6)
signalAxe.grid(True)
...
penLn,     = plt.plot([],[],'k-',linewidth=6,alpha=0.5)
penRLn,    = plt.plot([],[],'b-')
penILn,    = plt.plot([],[],'r-')
